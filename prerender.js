...
async function prerender() {
  try {
    ...
    const buildTemplate = fs.readFileSync(resolve(dir, 'dist/client/index.html'), {encoding: 'utf-8'})
    console.log(buildTemplate)
    const renderer = new Renderer(buildTemplate)
    ...
    Object.entries(renderer.pages).forEach(([pathname, page]) => {
      ...
      fs.writeFileSync(resolve(dir, filePath), body)
